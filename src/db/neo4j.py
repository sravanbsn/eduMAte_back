...
class Neo4jDatabase:
    ...
    def get_session(self):
        if self._driver is None:
            logger.warning("Attempted to get session without active driver. Connecting...")
            self.connect()
        return self._driver.session()


neo4j_db = Neo4jDatabase(uri="", user="", password="") # Injected later in config/startup

def init_neo4j_constraints(db: Neo4jDatabase):
    """
    ...
    try:
        with db.get_session() as session:
            session.run(query)
            logger.info("Neo4j constraints successfully initialized (Concept uniqueness).")
    except Exception as e:
